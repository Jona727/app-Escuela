...
const GestionAcademica = () => {
      ...
      <div className="card shadow-sm p-4">
        ...
        <form className="mb-3">
          <div className="row g-2">
            <div className="col-md-12">
              ...
              <input type="text" className="form-control" placeholder="agregar curso" value={cursoNombre} onChange={(e) => setCursoNombre(e.target.value)} />
            </div>
          </div>
        ...
        <div className="list-group">
          {cursos.length === 0 ? (
            ...
            cursos.map((curso) => (
              <div key={curso.id} className="list-group-item d-flex justify-content-between align-items-center">
        ...
        <form className="mb-3">
          <div className="row g-2 align-items-end">
            <div className="col-md-8">
              ...
              <input type="text" className="form-control" placeholder="Nombre de la materia" value={nuevaCarrera} onChange={(e) => setNuevaCarrera(e.target.value)} />
            </div>
            <div className="col-md-4 d-flex align-items-end">
              <button type="button" className="btn btn-success w-100" onClick={crearCarrera}>
                <Plus size={16} className="me-2" /> Crear
              </button>
            </div>
          </div>
        </form>
        <div className="list-group">
          {carreras.length === 0 ? (
            ...
            carreras.map((carrera) => (
              <div key={carrera.id} className="list-group-item d-flex justify-content-between align-items-center">
                <p className="fw-bold mb-0">{carrera.name}</p>
                <button className="btn btn-danger btn-sm" onClick={() => eliminarCarrera(carrera.id)}>
                  <Trash2 size={16} />
                </button>
              </div>
            ))
  ...
  const renderAsignaciones = () => {
    const agrupadas = asignaciones.reduce((acc, asignacion) => {
      ...
      acc[cursoId].materias.push({ id: asignacion.id, carrera_id: asignacion.carrera_id, carrera_nombre: asignacion.carrera_nombre });
      return acc;
              ...
              <div key={cursoId} className="list-group-item">
                <div className="d-flex justify-content-between align-items-center">
                  <div>
                    ...
                    <small className="text-muted">{grupo.materias.length} materia{grupo.materias.length !== 1 ? "s" : ""} asignada{grupo.materias.length !== 1 ? "s" : ""}</small>
                  </div>
                </div>
                <div className="d-flex flex-wrap gap-2 mt-2">
                  {grupo.materias.map((materia) => (
                    <div key={materia.id} className="badge bg-primary d-flex align-items-center gap-2">
                      ...
                      <button type="button" className="btn-close btn-close-white p-1" onClick={() => eliminarAsignacion(materia.id)} title="Eliminar asignación"></button>
                    </div>
                  ))}
                </div>
              </div>
    ...
    <div className="container-fluid bg-light min-vh-100 py-4">
      <div className="container">
        <div className="text-center mb-3">
          ...
          <h3 className="fw-bold">Gestión Académica</h3>
          <p className="text-muted">Administra materias, cursos y sus asignaciones</p>
        </div>


        <ul className="nav nav-pills justify-content-center mb-4">
          <li className="nav-item">
            <button className={`nav-link ${activeTab === "materias_cursos" ? "active" : ""}`} onClick={() => setActiveTab("materias_cursos")}>📚 Materias y Cursos</button>
          </li>
          <li className="nav-item">
            <button className={`nav-link ${activeTab === "asignaciones" ? "active" : ""}`} onClick={() => setActiveTab("asignaciones")}>🔗 Asignaciones</button>
          </li>
        </ul>
